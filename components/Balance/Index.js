...
export default function Index(){
    ...
    const [balanceTransactionList, setBalanceTransactionList] = useState([])
    ...
    const getPaymentHistory = async () => {
        ...
        const res = await apiHelper('transaction/balance', 'GET', null, headers)
        if(res.data.status){
            let balanceTransactions = res.data.data;
            setBalanceTransactionList(balanceTransactions)
            set_loading(false)
            console.log('paymentHistories list === ', balanceTransactions)
        }
    }
    ...
    return <div className="pb-4">
        <div className="flex justify-between items-center ml-auto py-2">
            <div>
                <span className="px-3  font-bold mr-5 w-48">Balance Transactions</span>
            </div>
            <div className="mr-7 font-bold">
                <p>Balance: <span className=" text-green-500">${balance.toFixed(2)}</span></p>
            </div>
            {/* <div>
        ...
        <tbody>
            ...
            {balanceTransactionList.map(t => {
                return <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td className={`px-6 py-4 ${t.amount.includes("+") ? 'text-green-600' : 'text-red-600' }`}>
                        {t.amount}
                    </td>
    ...
    {loading && <Loader />}
    {!loading && balanceTransactionList.length == 0 && <p className=" text-gray-700 p-4 text-center font-semibold text-xl">No record found</p>}
    </div>
}