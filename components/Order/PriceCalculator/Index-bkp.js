         ...
         <div className="p-4 border">
             ...
             <div className="p-4 border">
                 ...
                 <div className="flex items-center mb-3">
                     <div className="flex flex-1 flex-row h-8 w-full rounded-lg relative bg-transparent">
                         ...
                         <span id="pagenumber" className="focus:outline-none text-center w-14 mx-1 border border-gray-300 text-md hover:text-black focus:text-black  md:text-basecursor-default flex items-center text-gray-700 " name="custom-input-number">{pages}</span>
                 ...
                 <select id="countries" className=" border border-gray-400 text-gray-900 text-sm rounded-sm focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 mb-3" onChange={(e) => set_spacing(e.target.value)}>
                 {spacings.map(l => {
                    if(l.id == spacing){
                 ...
                 <select id="deadline" className="border border-gray-400 text-gray-900 text-sm rounded-sm focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 mb-1" onChange={(e) => set_deadline_id(e.target.value)}>
                 {deadlines.map(l => {
                     if(l.id == deadline_id){
                 ...
                 <select id="writer-category" className="border border-gray-400 text-gray-900 text-sm rounded-sm focus:ring-blue-500 focus:border-blue-500 block w-full p-1.5 mb-3" onChange={(e) => set_writer_category_id(e.target.value)}>
                 {writerCategories.map(l => {
                       if(l.id == writer_category_id){
                 ...
                 <div className="flex items-center mb-3">
                     <div className="flex flex-1 flex-row h-8 w-full rounded-lg relative bg-transparent">
                         ...
                         <span id="slidenumber" className="focus:outline-none text-center w-14 mx-1 border border-gray-300 text-md hover:text-black focus:text-black  md:text-basecursor-default flex items-center text-gray-700 " name="custom-input-number"> {slides} </span>
                        ...
                        return (<div key={item.id}>
                            {item.service_name == 'Plagiarism report' ? 
                                <div className="p-2 bg-yellow-50 border">
                                    {plagiarism_report ?
                                     <input checked className="mr-2" type="checkbox" name={item.service_name} id={item.service_name} onChange={() => set_plagiarism_report(plagiarism_report == 1 ? 0 : 1)} />
                                    :  <input className="mr-2" type="checkbox" name={item.service_name} id={item.service_name} onChange={() => set_plagiarism_report(plagiarism_report ? 0 : 1)} /> 
                                    }
                            ...
                            <div className="p-2 bg-yellow-50 border" >
                                {abstract_page ? 
                                <input checked className="mr-2" type="checkbox" name={item.service_name} id={item.service_name} onChange={() => set_abstract_page(abstract_page ? 0 : 1)} />
                                : <input className="mr-2" type="checkbox" name={item.service_name} id={item.service_name} onChange={() => set_abstract_page(abstract_page ? 0 : 1)} />
                                }
                                <label htmlFor={item.service_name}>{item.service_name} (${item.price})</label><br />
                            </div> : null}
                            ...
                            <div className="p-2 bg-yellow-50 border">
                                {high_priority ? 
                                <input checked className="mr-2" type="checkbox" name={item.service_name} id={item.service_name} onChange={() => set_high_priority(high_priority ? 0 : 1)} />
                                : <input className="mr-2" type="checkbox" name={item.service_name} id={item.service_name} onChange={() => set_high_priority(high_priority ? 0 : 1)} />
                                }
                                <label htmlFor={item.service_name}>{item.service_name} (${item.price})</label><br />
                            </div> : null}
                        </div>
                 ...
                 <div className="mb-3 border px-2 py-3">
                     ...
                     <div className="flex justify-end">
                         <input className="p-1 border-gray-400 border w-full"  type="text" placeholder="Enter promo code" />
                         <button className="bg-[#f8cc00] px-4 ">Apply</button>
                     </div>
                    <span className=" text-red-500">Invalid Coupone/Referral Code.</span>
                 </div>    
             ...
             <div>
                 ...
                 <div className="flex justify-between items-center mt-7 p-4">
                     <span className="flex bg-yellow-50 px-4 py-2 font-normal border" onClick={() => handleSteps(false)}>
                         <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" className="w-6 h-6">
                             <path strokeLinecap="round" strokeLinejoin="round" d="M6.75 15.75L3 12m0 0l3.75-3.75M3 12h18" />
                           </svg>                              
                         <span className="ml-2">Previous</span>
                     </span>
                     <span className="flex bg-[#f8cc00] px-4 py-2 font-normal" onClick={() => handleSteps(true)}>
